        ...
        rounded-md p-1 bg-white shadow-sm'>
          {dataArray.map((tab) => (
            <button
              ...
              key={tab.label}
              className={`cursor-pointer flex items-center px-3 py-1 text-sm rounded-sm 
              transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black' : 'text-gray-500 hover:text-black'}`}
            >
          ...
          {dataArray.find((item) => item.label === currentTab).value.map((user) => (
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md'>
              ...
              <div className='flex-1'>
                ...
                <p className='text-slate-600'>{user.bio.slice(0, 30)}...</p>
                <div className='flex max-sm:flex-col gap-2 mt-4'>
                  ...
                  {currentTab === 'Following' && (
                    <button className='w-full text-sm rounded bg-slate-100 hover:bg-slate-200 text-black
